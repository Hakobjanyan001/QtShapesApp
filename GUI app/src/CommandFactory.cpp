...
/*
    CommandFactory – Central command dispatcher
    Recognizes:
      create_* prefixed commands (create_triangle, create_line, etc.)
      delete / remove
      execute_file
      connect (partially implemented)
...
std::unique_ptr<Command> CommandFactory::createCommand( class MainWindow* window, const std::vector<std::string>& etokens) {
    ...
    if( firstLower == "execute_file" && etokens.size() >= 2 ) {
		if (etokens.size() >= 2) {
	       return std::make_unique<ExecuteFileCommand>(window, 
